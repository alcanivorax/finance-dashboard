...
const mockRecords = [
  { id: "1", amount: 100, type: "INCOME", category: "Salary", date: new Date("2024-01-15"), userId: 1 },
  { id: "2", amount: 50, type: "EXPENSE", category: "Food", date: new Date("2024-01-20"), userId: 1 },
  { id: "3", amount: 200, type: "INCOME", category: "Freelance", date: new Date("2024-02-10"), userId: 1 },
];
...
describe("Record Filtering Logic", () => {
  ...
  describe("Filter by date range", () => {
    it("filters records within date range", () => {
      ...
      const filtered = mockRecords.filter((r) => r.date >= startDate && r.date <= endDate);
      expect(filtered).toHaveLength(2);
    });
  ...
  describe("Combined filters", () => {
    it("filters by type and category", () => {
      const filtered = mockRecords.filter((r) => r.type === "INCOME" && r.category === "Salary");
      expect(filtered).toHaveLength(1);
      expect(filtered[0].id).toBe("1");
    });
    ...
    it("filters by type and date range", () => {
      const filtered = mockRecords.filter(
        (r) => r.type === "INCOME" && r.date >= new Date("2024-01-01") && r.date <= new Date("2024-01-31")
      );
      expect(filtered).toHaveLength(1);
      expect(filtered[0].id).toBe("1");
    });
...
describe("CRUD Operations", () => {
  ...
  describe("Create", () => {
    it("adds new record", () => {
      const newRecord = { id: "4", amount: 75, type: "EXPENSE" as const, category: "Entertainment", date: new Date("2024-02-20"), userId: 1 };
