...
const mockRecords = [
  { id: "1", amount: 1000, type: "INCOME", category: "Salary", date: new Date("2024-01-15"), userId: 1 },
  { id: "2", amount: 200, type: "EXPENSE", category: "Food", date: new Date("2024-01-20"), userId: 1 },
  { id: "3", amount: 500, type: "INCOME", category: "Freelance", date: new Date("2024-02-10"), userId: 1 },
  { id: "4", amount: 150, type: "EXPENSE", category: "Transport", date: new Date("2024-02-15"), userId: 1 },
];
...
describe("Dashboard Summary", () => {
  ...
  it("calculates net balance correctly", () => {
    const totalIncome = mockRecords.filter((r) => r.type === "INCOME").reduce((sum, r) => sum + r.amount, 0);
    const totalExpense = mockRecords.filter((r) => r.type === "EXPENSE").reduce((sum, r) => sum + r.amount, 0);
...
describe("Dashboard Trends", () => {
  it("groups records by month", () => {
    const trendsByMonth = mockRecords.reduce((acc, r) => {
  ...
  it("sorts months chronologically", () => {
    const monthKeys = Object.keys(mockRecords.reduce((acc, r) => {
      const month = `${r.date.getFullYear()}-${String(r.date.getMonth() + 1).padStart(2, "0")}`;
      acc[month] = true;
      return acc;
